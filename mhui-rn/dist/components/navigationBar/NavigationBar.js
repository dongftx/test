...
class NavigationBar extends Component {
  ...
  constructor(props, context) {
    ...
    const _initWindow = Dimensions.get('window');

    if (_initWindow.width > _initWindow.height) {
      this.initWindow = { ..._initWindow,
    ...
    this.state = {
      navWidth: Dimensions.get('window').width
    };
  }
  ...
  orientationDidChange = orientation => {
    if (orientation === 'LANDSCAPE') {
      const width = Platform.OS === 'ios' ? Dimensions.get('window').width : this.initWindow.height;
      this.setState({
        navWidth: width
      });
    } else {
      const width = Platform.OS === 'ios' ? Dimensions.get('window').width : this.initWindow.width;
      this.setState({
        navWidth: width
      });
    }
